...
filepath <- here("outputs","WorkingDataFrames","LBB_filtered.csv")
...
filepath_meta <- here("raw_data","spatial_data", "LBBG", "Seabird_Tagging_Metadata.csv")
...
df_CPlocs_optionA1 <- data.frame(ID = df_metadata$Bird_ID,
...
threshold_dist <- 600 #distance in meters
...
shapefilepath <- here("raw_data","spatial_data","map_layers", "Gugh", "Gugh.shp")
...
CP_shape <- st_read(dsn = shapefilepath, crs = 4326) %>%
  ...
  mutate(CP = "CP", Population = "Gugh") # add a common column name for later indexing and population column

CP_shape <- st_buffer(CP_shape, dist = 200) #add buffer of 200m to remove positions when resting on rocks etc.

ggplot() + geom_sf(data = CP_shape_buff, fill = "red") + geom_sf(data = CP_shape)
...
plot_alltripdurations = ggplot(df_alltripdurations, aes(x = as.numeric(Trip_duration)))+
  geom_histogram(binwidth = 5)+ # each bar = 5 mins
...
##Save trip duration df
saveRDS(df_alltripdurations, "outputs/SummaryDataFrames/LBB_alltripdurations.rds")

## Save trips durations plot
...
threshold_time <- 60 #time in minutes
...
df_tripmetrics_comp <- subset(df_tripmetrics, Completetrip == "Complete")

df_tripmetrics_ind_comp <- df_tripmetrics_comp %>%
  group_by(across(all_of(c(grouping_factors_poplevel, grouping_factors_indlevel)))) %>% 
  summarise(Trip_duration_ind_mean = mean(as.numeric(Trip_duration, units = sampleRateUnits), na.rm = T), # in hours
            Trip_duration_ind_se = se(as.numeric(Trip_duration, units = sampleRateUnits)),
            Trip_duration_ind_min = min(as.numeric(Trip_duration, units = sampleRateUnits)),
            Trip_duration_ind_max = max(as.numeric(Trip_duration, units = sampleRateUnits)),
            Total_distance_ind_mean = mean(Total_distance)/1000,
            Total_distance_ind_se = se(Total_distance)/1000,
            Total_distance_ind_min = min(Total_distance)/1000,
            Total_distance_ind_max = max(Total_distance)/1000,
            Max_distance_ind_mean = mean(Max_distance)/1000,
            Max_distance_ind_se = se(Max_distance)/1000,
            N_Trips = length(unique(TripID)))
df_tripmetrics_ind_comp

## population level summary = 1 row per population

df_tripmetrics_summary_pop <- df_tripmetrics_ind_comp %>%
...
map_trips <- map_base + 
  ...
  geom_path(data = df_trips, aes(x = Lon, y = Lat, col = as.factor(trip_num)), alpha = 0.8, linewidth = 0.5 ) +
...
ggsave(plot = plot_tripmetrics, filename = paste0(species_code, "_pop_tripmetrics.tiff"),
       device = device,
       path = out_path ,units = "mm", width = 200, height = 175, dpi = 300,   
)
...
filename_tripmetrics_summary_ind_out <- paste0(species_code, "_tripmetrics_summary_ind")

## file path & name for df_trips_withmetrics
filename_dftrips_withmetrics_out <- paste0(species_code, "_withtripmetrics")
...
write_csv(df_tripmetrics_ind_comp, file = here(filepath_summary_out, paste0(filename_tripmetrics_summary_ind_out, ".csv")))

write_csv(df_trips_withmetrics, file = here(filepath_dftrips_out, paste0(filename_dftrips_withmetrics_out, ".csv")))
