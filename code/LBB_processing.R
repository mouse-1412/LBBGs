...
filepath <- here("raw_data", "spatial_data", "LBBG", "LBB") 
...
IDstart <- 14 #start position of the ID in the filename 
IDend <- 17 #end position of the ID in the filename
...
read_lines(test, n_max = 10)  # change n_max to change the number of rows to read in
...
datetime_colnames <- "DateTime" # or c("DateTime") 
...
metadatatimezone <- "GMT" #specify timezone used for metadata
...
names(df_metadata)
df_metadata <- subset(df_metadata, Species == "LBB") #removing ESH from metadata
...
df_metadataslim <- data.frame(ID = as.character(df_metadata$Bird_ID), # compulsory column
                              TagID = as.character(str_sub(string = df_metadata$Logger_ID, start = 7, end = 10)), # isolating Tag number to join with raw data
                              DeployDate_local = df_metadata$Date, # compulsory column (set to NA if not relevant)
                              DeployTime_local = df_metadata$Time_Capture, # compulsory column (set to NA if not relevant)
                              RetrieveDate_local = df_metadata$Retrieval_date, # compulsory column (set to NA if not relevant)
                              RetrieveTime_local = df_metadata$Retrieval_time, # compulsory column (set to NA if not relevant)
                              CPY = df_metadata$Nest_Lat,
                              CPX = df_metadata$Nest_Lon,
                              Species = "LBB",
                              Population = df_metadata$Island,
                              Sex = df_metadata$Sex,
                              BillLength = df_metadata$Bill_length,
                              BillDepth = df_metadata$Bill_depth,
                              WingLength = df_metadata$Wing_length,
                              Weight = df_metadata$Weight,
                              BreedStage = df_metadata$Breed_stage)
...
transform_crs <- 4326 # Suggest changing if using own data
...
filepath_dfout <- here("outputs","WorkingDataFrames")
...
filter_netdisp_dist <- 180000
filter_netdist_units <- "m" # e.g., "m", "km"
...
filepath_filtered_out <- here("outputs","WorkingDataFrames")
